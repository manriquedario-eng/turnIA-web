...
async function createProfessionalCancellationRow(params: {
  ...
  const { data: existing } = await supabase
    ...
    .eq('message_type', 'professional_appointment_cancelled')
    .eq('channel', 'email')
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      message_type: 'professional_appointment_cancelled',
      channel: 'email',
...
async function finishProfessionalCancellation(params: {
  messageRowId: string;
  ok: boolean;
  ...
  await supabase
    ...
    .update({
      ...
      error_message: params.ok
        ? null
        : params.errorMessage?.slice(0, 500) ?? 'No se pudo enviar el aviso de cancelación.',
      updated_at: new Date().toISOString(),
    })
    .eq('id', params.messageRowId);
}

export async function notifyProfessionalAboutCancellationByToken(token: string): Promise<void> {
  ...
  if (
    ...
    !['cancelled', 'cancelado'].includes(String(appointment.status ?? '').toLowerCase())
  ) {
    return;
  }
  ...
  const subject = `Turno cancelado — ${patientName} — ${dateLabel}`;
  const text = [
    'TurnIA',
    '',
    ...
    `Hora: ${timeLabel}`,
    '',
    'El turno ya figura cancelado en la Agenda de TurnIA.',
    agendaUrl,
  ].join('\n');
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:520px;margin:0 auto;color:#111827;">
      ...
      <h1 style="font-size:20px;">Turno cancelado</h1>
      <p><strong>${safePatientName}</strong> canceló su turno.</p>
      <p>Fecha: <strong>${dateLabel}</strong><br/>Hora: <strong>${timeLabel}</strong></p>
      <p>El turno ya figura cancelado en la Agenda de TurnIA.</p>
  ...
  try {
    ...
    await finishProfessionalCancellation({
      messageRowId,
      ok: result.ok,
    ...
    await finishProfessionalCancellation({
      messageRowId,
      ok: false,
      errorMessage: 'Error inesperado al enviar el aviso de cancelación por email.',
    });

    console.error('Professional cancellation email failed', {
      appointmentId: appointment.id,
    });
  }
}
