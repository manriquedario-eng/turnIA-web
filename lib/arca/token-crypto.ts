// Cifrado en reposo de secretos de ARCA: private_key_ciphertext en
// arca_connections; token_ciphertext/sign_ciphertext en arca_auth_tickets.
// AES-256-GCM con APIs estándar de Node (`node:crypto`) — mismo diseño que
// lib/mercadopago/token-crypto.ts y lib/google/token-crypto.ts, pero con su
// PROPIA clave: ARCA_TOKEN_ENCRYPTION_KEY nunca se comparte ni se deriva de
// MP_TOKEN_ENCRYPTION_KEY, GOOGLE_TOKEN_ENCRYPTION_KEY ni ningún otro
// secreto existente.
//
// server-only: este módulo nunca debe poder terminar en el bundle del
// browser (maneja la clave de cifrado de la clave privada del certificado
// digital del profesional y de sus tickets de acceso a ARCA).
//
// Variable de entorno requerida (server-side únicamente, nunca
// NEXT_PUBLIC_): ARCA_TOKEN_ENCRYPTION_KEY — base64 de exactamente 32 bytes
// aleatorios. Generarla, por ejemplo:
//   node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"
//
// Formato persistido, versionado (permite rotar el esquema a futuro sin
// romper filas viejas) — idéntico al de Google/Mercado Pago:
...
// fuera un secreto válido. Nunca hay fallback a texto plano: si el cifrado
// falla al guardar una conexión o un ticket, la operación se aborta sin
// persistir nada (ver lib/arca/wsaa.ts).
//
...
/**
 * Decodifica y valida `ARCA_TOKEN_ENCRYPTION_KEY`. Devuelve `null` (nunca
...
function loadEncryptionKey(): Buffer | null {
  const raw = process.env.ARCA_TOKEN_ENCRYPTION_KEY;
  if (!raw) return null;
...
export function isArcaTokenEncryptionConfigured(): boolean {
 ...
 * de IV/tag acá — eso lo hace `decryptArcaSecret`, que es quien puede fallar
 * cerrado con detalle.
 */
export function isEncryptedArcaSecret(value: string | null | undefined): value is string {
...
export type ArcaCryptoResult<T> =
  | { ok: true; data: T }
  | { ok: false; reason: 'not_configured' | 'encryption_failed' | 'decryption_failed' };

/**
 * Cifra `plaintext` (una clave privada, un token o un sign de ARCA) con
 ...
 */
export function encryptArcaSecret(plaintext: string): ArcaCryptoResult<string> {
 ...
 */
export function decryptArcaSecret(stored: string): ArcaCryptoResult<string> {
  const key = loadEncryptionKey();
  if (!key) return { ok: false, reason: 'not_configured' };

  if (!isEncryptedArcaSecret(stored)) return { ok: false, reason: 'decryption_failed' };
