// Cifrado en reposo de los tokens OAuth de Google (access_token/refresh_token
// en google_oauth_connections). AES-256-GCM con APIs estándar de Node
// (`node:crypto`) — no se implementa criptografía propia.
//
...
// NEXT_PUBLIC_): GOOGLE_TOKEN_ENCRYPTION_KEY — base64 de exactamente 32
// bytes aleatorios. Generarla, por ejemplo:
//   node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"
// NUNCA reutilizar GOOGLE_CLIENT_SECRET, tenant_id, user_id ni ningún otro
// valor derivable como clave.
//
...
// sólo la clave lo es. No se agregan columnas separadas para IV/tag.
//
...
// fuera un token válido.
//
// Nunca se loguea: texto plano, ciphertext completo, la clave, el tag ni el
// IV. Los únicos logs que emite este archivo (ninguno hoy) deberían limitar
// a un motivo técnico corto, igual que el resto de lib/google/.
...
/**
 * Decodifica y valida `GOOGLE_TOKEN_ENCRYPTION_KEY`. Devuelve `null` (nunca
 ...
 * de IV/tag acá — eso lo hace `decryptGoogleToken`, que es quien puede
 * fallar cerrado con detalle.
 */
export function isEncryptedGoogleToken(value: string | null | undefined): value is string {
...
export type GoogleTokenCryptoResult<T> =
  | { ok: true; data: T }
  | { ok: false; reason: 'not_configured' | 'encryption_failed' | 'decryption_failed' };

/**
 * Cifra `plaintext` (un access_token o refresh_token de Google) con
 ...
 */
export function encryptGoogleToken(plaintext: string): GoogleTokenCryptoResult<string> {
 ...
 */
export function decryptGoogleToken(stored: string): GoogleTokenCryptoResult<string> {
  const key = loadEncryptionKey();
  if (!key) return { ok: false, reason: 'not_configured' };

  if (!isEncryptedGoogleToken(stored)) return { ok: false, reason: 'decryption_failed' };
